...
export default function PublicProfilePage() {
  ...
  const handleContact = () => {
    setContacted(true);
    alert(`Persetujuan anonim berhasil! Jalur komunikasi langsung dibuka. Menghubungkan Anda dengan ${profile?.fullName} via WhatsApp...`);
  };
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 border border-slate-200/50 dark:border-slate-800/50 backdrop-blur-md rounded-3xl p-8 shadow-xl space-y-8">
            ...
            <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-200/50 dark:border-slate-800/50 rounded-2xl p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-sm font-extrabold text-slate-700 dark:text-slate-300">
                  {profile.lastDonation ? new Date(profile.lastDonation).toLocaleDateString(dateLocale, { day: "numeric", month: "long", year: "numeric" }) : "Belum Pernah"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              ...
              {contacted ? "Hubungi via WhatsApp (Sudah Terhubung)" : "Hubungi via WhatsApp (Persetujuan Anonim)"}
            </button>
            <p className="text-center text-[11px] text-slate-400 dark:text-slate-500 mt-3 leading-relaxed">
              *BloodConnect melindungi privasi Anda. Nomor telepon dan kontak hanya akan dibagikan kepada pihak yang bersangkutan setelah persetujuan diberikan.
            </p>
          </div>
